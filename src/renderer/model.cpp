...
void Model::traverseNode(unsigned int nextNode, Matrix4x4 matrix)
{
	...
	if (node.find("translation") != node.end())
	{
		...
		translation = Vector3::MakeVector3(transValues);
	}
	...
	if (node.find("rotation") != node.end())
	{
		...
		rotation = Quaternion::MakeQuaternion(rotValues);
	}
	...
	if (node.find("scale") != node.end())
	{
		...
		scale = Vector3::MakeVector3(scaleValues);
	}
	...
	if (node.find("matrix") != node.end())
	{
		...
		matNode = Matrix4x4::MakeMatrix4x4(matValues);
	}
	...
	Matrix4x4 matNextNode = matrix * matNode * trans * rot * sca;
...
std::vector<unsigned char> Model::getData()
{
	...
	bytesText = Loader::LoadItemAsString((fileDirectory + uri).c_str());
